...
namespace SrtsWeb.Presenters.Reporting
{
    public sealed class RptDDForm771Presenter
    {
        ...
        public void GetOrder771(string orderNumbs)
        {
            ...
            if (_view.mySession.MySite.SiteType.ToLower() == "lab")
                dt = orderRepository.Get711DataByLabCode(_view.mySession.MySite.SiteCode, orderNumbs, _view.mySession.ModifiedBy);
            else
                dt = orderRepository.Get711DataByLabCode(null, orderNumbs, _view.mySession.ModifiedBy);

            if (!(dt == null || dt.Rows.Count <= 0))
            {
                dt.TableName = "OrderData";

                var bc = new SrtsWeb.BusinessLayer.Concrete.GenerateBarCodes(new BarcodeLib.Barcode());
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["ONBarCode"] == null)
                    {
                        using (Image tImage = bc.GenerateBarCode(String.Format("*{0}*", dr["OrderNumber"].ToString())))
                        {
