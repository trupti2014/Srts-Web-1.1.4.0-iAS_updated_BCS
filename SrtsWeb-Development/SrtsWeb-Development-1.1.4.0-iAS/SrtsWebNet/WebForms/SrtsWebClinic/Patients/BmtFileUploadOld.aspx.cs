...
//            var fileName = this.hfFile.Value.Substring(this.hfFile.Value.LastIndexOf(@"\") + 1);
            string saveInfo = string.Format("{0}{1}", saveDir, tempFileName);
            if (this.fUpload.HasFile)
            {
                this.fUpload.SaveAs(saveInfo);
            }
        }
...
//                }
                ...
                //var myFile = string.Format("{0}.{1}{2}{3}{4}_proc.csv", fileName, DateTime.Now.Day, DateTime.Now.Month, DateTime.Now.Year, DateTime.Now.Minute);
        ...
        private string ProcessLine(string[] tmpArray)
        {
            ...
            if (string.IsNullOrEmpty(tmpArray[0]))
            {
                eMessage = "No Data";
                return eMessage;
            }
            ...
            ine.DateLastModified = DateTime.Now;
        ...
        {
            get { return TmpFileName.Value; }
            set
            {
                string tmpString = string.Format("{0}_{1}_{2}_{3}_{4}.csv", mySession.MyClinicCode, DateTime.Now.Day, DateTime.Now.Month, DateTime.Now.Year, DateTime.Now.Hour);
                TmpFileName.Value = tmpString;
            }
        }
