...
namespace SrtsWeb.BusinessLayer.Presenters.Individuals
{
    public sealed class IndividualManagementAddPresenter
    {
        ...
        public Boolean SearchIndividualDmdc()
        {
            ...
            var dmdc = new DmdcService();
            var list = new List<DmdcPerson>();

            if (idNumType.Equals("SSN"))
                list = dmdc.DoDmdcBySsn(idNum).ToList();
            else
                list = dmdc.DoDmdcByDodId(idNum).ToList();
            ...
            //var indList = new List<IndividualEntity>();
            ...
            _view.MiddleName = d.PnMiddleName;
            ...
            if (!String.IsNullOrEmpty(d._DmdcPersonnel.PnCategoryCode))
            {
                ...
                switch (d._DmdcPersonnel.PnCategoryCode)
                {
                    case "00":
                    case "32":
                    case "65":
                    ...
                    case "W":
                    case "Y":
                        d._DmdcPersonnel.ServiceCode = "K";
                        break;
                }
                ...
                if (!String.IsNullOrEmpty(d._DmdcPersonnel.PnCategoryCode))
                {
                    ...
                    if (!String.IsNullOrEmpty(d._DmdcPersonnel.PayPlanCode) && !String.IsNullOrEmpty(d._DmdcPersonnel.PayGrade))
                    {
                        ...
                        if (d._DmdcPersonnel.PayPlanCode.Equals("ME") || d._DmdcPersonnel.PayPlanCode.Equals("MO") || d._DmdcPersonnel.PayPlanCode.Equals("MW"))
                            _view.RankTypeSelected = String.Format("{0}{1}", d._DmdcPersonnel.PayPlanCode.Substring(1), d._DmdcPersonnel.PayGrade);
