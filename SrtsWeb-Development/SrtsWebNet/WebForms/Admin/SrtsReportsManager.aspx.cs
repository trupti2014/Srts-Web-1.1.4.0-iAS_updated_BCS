    ...
    public partial class SrtsReportsManager : PageBase, ISiteMapResolver, IReportsManagerView
    {
        ...
        private void BuildReportsList()
        {
                ...
                )
            {
                ...
                lstReports016.Value = "/LabRoutingForm";
                ...
                )
            {
                ...
                lstReports017.Text = "Reprint Form";
        ...
        protected void BindGridviewOrderAddresses()
        {
            if (OrderAddressesList != null)
            {
                if (OrderAddressesList.Count > 0)
                {
                    ...
                    using (StreamWriter sw = File.CreateText(HttpContext.Current.Server.MapPath("log12.txt")))
                    {
                        sw.WriteLine("OrderAddressesList.Count: " + OrderAddressesList.Count);
                    }
                }
                else
                {
        ...
        public void GetOrderDataForLabel(string orderNumber, string addressType)
        {
            ...
            if (_order != null)
            {
                ...
                using (StreamWriter sw = File.CreateText(HttpContext.Current.Server.MapPath("log11.txt")))
                {
                    sw.WriteLine("_order: " + _order.FirstName);
                    sw.WriteLine("_order: " + _order.OrderNumber);
                    sw.WriteLine("_order: " + patientOrderNumbers.Count);
                }
        ...
        protected void btnSubmitTop_Click(object sender, EventArgs e)
        {
            ...
            if (ddlSelectLabel.SelectedValue.ToString() == "Avery5160")
            {
                rptTitle = "Avery 5160";
                _documentID = "15441427";
            }
            else if (ddlSelectLabel.SelectedValue.ToString() == "SingleLabel")
            {
                rptTitle = "Single Label";
                _documentID = "15441434";

            }
            string ordernumberLabel = "";
            if (!(tbSingleReadScan.Text.IsNullOrEmpty()))
            {
                ordernumberLabel = tbSingleReadScan.Text.Trim();
            }
            else if (OrderAddressesList != null && OrderAddressesList.Count > 0)
            {
                foreach (GridViewRow row in gvOrderAddresses.Rows)
                {
                    if (ordernumberLabel != "")
                        ordernumberLabel += ",";
                    Label OrderNumber = (Label)row.FindControl("OrderNumber");
                    ordernumberLabel += OrderNumber.Text.Trim();
                }
            }
            if (ordernumberLabel != "")
            {
                string OrderNumberJsonParameter = "{\"@type\":\"prompt\",\"id\": 0,\"answer\": {\"@type\":\"Text\",\"values\": {\"value\": [\"" + ordernumberLabel + "\"]}}}";

                _jsonBody = "{\"parameters\": {\"parameter\": [" +
                                OrderNumberJsonParameter +
                            "]}}";
                ...
                using (StreamWriter sw = File.CreateText(HttpContext.Current.Server.MapPath("log3.txt")))
                {
                    ...
                    sw.WriteLine("_documentID: " + _documentID);
        ...
        private void getLogonTokenFromBCS()
        {
            try
            {
        ...
        private void displayPDFinIFrame(string _pdfFile)
        {
            try
            {
                ...
                if (!_pdfFile.IsNullOrEmpty())
                {
                    ...
                    if (File.Exists(src))
                    {
                        string LabelURL = (string.Format("LabelReport.aspx?filename={0}", _pdfFile));
                        string s = "window.open('" + LabelURL + "', 'popup_window', 'width=1500,height=1000,left=100,top=100,resizable=yes');";
                        ScriptManager.RegisterStartupScript(this, GetType(), "script", s, true);

                        using (StreamWriter sw = File.CreateText(HttpContext.Current.Server.MapPath("log2.txt")))
                        {
                            sw.WriteLine("url: " + LabelURL);
                        }
                    }
                    else
                    {
